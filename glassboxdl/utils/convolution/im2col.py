...
def get_im2col_indices(x_shape, field_height, field_width, padding=1, stride=1):
    """
    Computes the advanced numpy indices needed to extract patches.
    """
    N, C, H, W = x_shape
...
def im2col_indices(x, field_height, field_width, padding=1, stride=1):
    """
    Extracts patches from the image tensor and flattens them into columns.
...
def col2im_indices(cols, x_shape, field_height=3, field_width=3, padding=1, stride=1):
    """
    Reconstructs an image tensor from the flattened columns.
    Crucial for routing gradients backward through the Convolution!
    """
    N, C, H, W = x_shape
    ...
    np.add.at(x_padded, (slice(None), k, i, j), cols_reshaped)  # type: ignore
